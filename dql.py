...
class DQL:
    ...
    def train(self, terminal_state, step):
        ...
        current_states = np.array([transition[0] for transition in minibatch])/ACTION_SPACE_SIZE
        current_qs_list = self.model.predict(current_states)

        # Get future states from minibatch, then query NN model for Q values
        # When using target network, query it, otherwise main network should be queried
        new_current_states = np.array([transition[3] for transition in minibatch])/ACTION_SPACE_SIZE
        ...
        self.model.fit(np.array(X)/ACTION_SPACE_SIZE, np.array(y), batch_size=MINIBATCH_SIZE, verbose=0, shuffle=False, callbacks=[self.tensorboard] if terminal_state else None)
    ...
    def get_qs(self, state):
        return self.model.predict(np.array(state).reshape(-1, *state.shape)/ACTION_SPACE_SIZE)[0]
